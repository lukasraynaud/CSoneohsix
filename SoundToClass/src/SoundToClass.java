...
public class SoundToClass {
	...
	private class GUImaker implements Runnable {
		...
		private SoundToClassGUI gui = null;
		private boolean done = false;
		public GUImaker(SoundToClass itc){ this.stc = itc; }
		public void run(){ gui = new SoundToClassGUI(stc); done = true; }
		public SoundToClassGUI getGUI(){ while(!done){ try{ Thread.sleep(1); }catch(InterruptedException ex){} } return gui;  }
	}
	...
	public void alert(String message, int icon){
		// GUI
		if(GUI != null){
			JOptionPane.showMessageDialog(GUI, message, "Alert", icon);
		}
		// Command line
		else{
			System.out.println(message);
	...
	public void error(String message, Exception ex){
		...
		if(GUI == null) Runtime.getRuntime().exit(-1);
	}
		...
		try{
			...
			while(length < bytes.length){
				...
				for(int i=0; ((i<chunk)&&(length<bytes.length)); i++){
					...
					if(GUI != null) GUI.setProgress(0.5 + progress);
				}
				out.write("}; }\n");
				numArrays++;
			}
